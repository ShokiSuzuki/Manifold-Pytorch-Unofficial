...
class PatchEmbed(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_channels=3, embed_dim=768):
        ...
        self.proj = nn.Conv2d(in_channels=self.in_channels, out_channels=self.embed_dim,
...
class Attention(nn.Module):
    def __init__(self, embed_dim=768, num_heads=12, qkv_bias=False, att_drop_rate=0.0, proj_drop_rate=0.0):
        ...
        self.attn_drop = nn.Dropout(p=att_drop_rate)
    ...
    def forward(self, x):
        ...
        att_weight = self.attn_drop((torch.matmul(q, k) * (self.head_dim ** -0.5)).softmax(dim=-1))
        att        = self.proj_drop(self.proj(torch.matmul(att_weight, v).transpose(1, 2).reshape(B, N, C)))
...
class Block(nn.Module):
    def __init__(self, embed_dim=768, num_heads=12, qkv_bias=False, mlp_ratio=4, drop_rate=0.0,
        ...
        self.attn_drop = DropPath(drop_path_rate) if drop_path_rate > 0. else nn.Identity()
        self.norm2 = norm_layer(embed_dim)
        self.mlp = MLP(in_features=embed_dim, hidden_features=int(embed_dim * mlp_ratio), act_layer=act_layer, drop_rate=drop_rate)

    def forward(self, x):
        att, att_weight = self.attn(self.norm1(x))
        x = x + self.attn_drop(att)
        x = x + self.attn_drop(self.mlp(self.norm2(x)))
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_channels=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=nn.GELU, weight_init='', patch_proj=False, pre_rollout=False, proj_dim=768, attmap_token='cls'):
        super().__init__()
        """
        Args:
            ...
            patch_proj (bool): project patch embedding (same dimensionality number) if True
            pre_rollout (bool): return attention weights at all layers if True
        """

        assert attmap_token in ('cls', 'dist',)
        ...
        self.blocks = nn.Sequential(*[
            Block(
                embed_dim=embed_dim, num_heads=self.num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop_rate=drop_rate,
                att_drop_rate=attn_drop_rate, drop_path_rate=dpr[i], norm_layer=norm_layer, act_layer=act_layer)
            for i in range(self.depth)])
        self.norm = norm_layer(embed_dim)

        self.pre_rollout = pre_rollout
        self.patch_proj  = nn.Linear(self.embed_dim, proj_dim) if patch_proj else nn.Identity()
    ...
    def forward_features(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if self.dist_token is None:
                tokens_eachlayer.append(self.patch_proj(x[:, 1:]))
            else:
                tokens_eachlayer.append(self.patch_proj(x[:, 2:]))

        x = self.norm(x)
        if self.dist_token is None:
            return x[:, 0], tokens_eachlayer
        else:
            return x[:, 0], x[:, 1], tokens_eachlayer
    ...
    def forward(self, x):
        ...
        if self.head_dist is None:
            x, patches = self.head(x[0]), x[1]
            return x, patches
        else:
            x, x_dist, patches = self.head(x[0]), self.head_dist(x[1]), x[2]
